...
import GeneralExportMenu from "@/components/review/GeneralExportMenu";
...
// ---------------------------------------------------------------------------
// Priority groups — same as ReviewWorkspace
// ---------------------------------------------------------------------------
  ...
  return (
    <ol className="space-y-0.5">
      {entries.map((e) => {
        ...
        const fieldLbl = e.field_name ? getLabel(e.field_name) : null;
...
export default function GeneralResultsWorkspace({ initial }: Props) {
  ...
  const [approvalStatus, setApprovalStatus] = useState<ApprovalStatus>(
    initial.candidate.approval_status,
  );
  ...
  // Column lookup — type + label by field name
  const colMap = Object.fromEntries(columns.map((c) => [c.name, c]));
  function colType(fieldName: string)  { return colMap[fieldName]?.type  ?? "text"; }
  function colLabel(fieldName: string) { return prettyLabel(fieldName); }

  // Ordered fields: preserve schema order when available, else backend order
  const orderedFields =
    columns.length > 0
      ? columns.map((c) => fieldMap[c.name]).filter((f): f is ExtractedField => f !== undefined)
      : initial.fields;

  const groups = PRIORITY_GROUPS
    .map((g) => ({ ...g, fields: orderedFields.filter(g.filter) }))
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {(approvalStatus === "approved" || approvalStatus === "rejected") && (
            <button
              onClick={() => handleApproval("needs_review")}
              disabled={approving}
          ...
          <GeneralExportMenu candidateId={candidate.id} />

          <Link href="/general" className="btn-secondary text-xs py-1.5">
            New session
          </Link>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody>
              {groups.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-10 text-center text-sm text-stone-400 dark:text-stone-500 italic">
                  ...
                  <>
                    ...
                    {group.fields.map((f) => (
                      <GeneralFieldRow
                        ...
                        columnType={colType(f.field_name)}
                        columnLabel={colLabel(f.field_name)}
        ...
        {showAudit && (
          <div className="border-t border-stone-100 dark:border-stone-800 px-6 py-4 max-h-96 overflow-y-auto">
            ...
            {!auditLoading && !auditError && (
              <AuditTimeline entries={auditEntries} getLabel={colLabel} />
            )}
          </div>
        )}
      </div>
