...
import { editDraft, generateSubmittalDraft, listDrafts } from "@/lib/api";
...
export function SubmittalDraftPanel({ candidateId, analysisRunId }: Props) {
  ...
  useEffect(() => {
    listDrafts(candidateId)
      .then((all) => {
        // Latest submittal for this specific analysis run.
        const match = all.find(
          (d) => d.draft_type === "submittal" && d.analysis_run_id === analysisRunId,
        );
        if (match) {
          setDraft(match);
          setState("has_draft");
  ...
  async function handleGenerate() {
    ...
    setError(null);
    try {
      const created = await generateSubmittalDraft(candidateId, analysisRunId);
      setDraft(created);
      setState("has_draft");
    } catch (err) {
  ...
  return (
    <div className="space-y-3">
      ...
      {isGenerating && (
        <div className="h-40 bg-stone-50 rounded border border-stone-200 flex items-center justify-center">
          <span className="text-sm text-stone-400 animate-pulse">Generating submittal…</span>
        </div>
      )}
      ...
      {!isGenerating && isEditing && (
        <textarea
          ...
          disabled={isSaving}
          className="w-full min-h-[200px] rounded border border-aubergine-300 bg-white px-4 py-3 text-sm text-stone-800 leading-relaxed resize-none focus:outline-none focus:ring-2 focus:ring-aubergine-400 disabled:opacity-50"
      ...
      {!draft && !isGenerating && (
        <p className="text-sm text-stone-400">
          No submittal yet. Click Generate Submittal to create a job-specific candidate
          presentation grounded in this analysis.
        </p>
      )}
    </div>
  );
