...
const STATUS_CONFIG: Record<string, { label: string; dot: string; text: string; border: string; bg: string }> = {
  extracted:  { label: "AI",         dot: "bg-aubergine-400",           text: "text-aubergine-900 dark:text-aubergine-300",        border: "border-aubergine-200 dark:border-aubergine-900",   bg: "bg-aubergine-50 dark:bg-aubergine-950/20"   },
  ...
  edited:     { label: "Edited",     dot: "bg-aubergine-700",           text: "text-aubergine-900 dark:text-aubergine-300",        border: "border-aubergine-200 dark:border-aubergine-900",   bg: "bg-aubergine-50 dark:bg-aubergine-950/20"   },
...
export default function GeneralFieldRow({ field, candidateId, columnType, columnLabel, onUpdate }: Props) {
  ...
  const inputCls = "w-full rounded border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 px-2.5 py-1.5 text-sm text-stone-900 dark:text-stone-100 focus:outline-none focus:border-aubergine-700 transition-colors";
            ...
            <div className="h-1 w-10 rounded-full bg-stone-200 dark:bg-stone-700 overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${
                  confidencePct >= 85 ? "bg-aubergine-700" : confidencePct >= 60 ? "bg-amber-400" : "bg-red-400"
                }`}
      ...
      <td className="py-3 pr-3 w-32 text-right">
        ...
        {!editing && (
          <div className="flex items-center justify-end gap-2 flex-wrap">
            ...
            <button onClick={startEdit} disabled={anyBusy} className="text-xs font-medium text-aubergine-800 dark:text-aubergine-400 hover:opacity-70 transition-opacity disabled:opacity-30">
              Edit
            </button>
          </div>
        )}
