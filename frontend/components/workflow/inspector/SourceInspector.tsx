...
const BOT_STATUS_META: Record<string, { dot: string; label: string; hint: string }> = {
  ...
  transcribing:          { dot: "bg-teal-400 animate-pulse",  label: "Transcribing…",        hint: "Call ended. Generating transcript." },
  ready:                 { dot: "bg-teal-500",                label: "Transcript ready",     hint: "Running extraction pipeline." },
  extracting:            { dot: "bg-teal-400 animate-pulse",  label: "Extracting…",         hint: "Running AI extraction." },
  complete:              { dot: "bg-emerald-500",             label: "Complete",             hint: "Workflow finished. Candidate ready." },
    ...
    <Field label="Zoom recording">
      <div className="flex items-center justify-between mb-1.5">
        ...
        <button
          ...
          onClick={load}
          disabled={loading}
          className="text-[10px] font-medium text-teal-600 hover:text-teal-700 disabled:opacity-40"
        >
        ...
        {sessions.map((s, i) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex-shrink-0 text-right">
                <p className={`text-[10px] font-medium ${s.status === "complete" ? "text-emerald-500" : s.status === "failed" ? "text-red-500" : "text-stone-400"}`}>
                  {meta.label}
                </p>
                {s.status === "complete" && s.candidate_id && (
                  <a href={`/review/${s.candidate_id}`} className="text-[10px] text-teal-600 hover:text-teal-700 font-medium">
                    Review →
                  </a>
                )}
              </div>
