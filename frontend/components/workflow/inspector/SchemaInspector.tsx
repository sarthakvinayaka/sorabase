...
export default function SchemaInspector({ id, data }: Props) {
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            {templates.map((tmpl) => (
              <button
                ...
                className="w-full text-left rounded-lg border border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-800 px-3 py-2 hover:border-teal-400 dark:hover:border-teal-600 transition-colors"
              >
      ...
      {columns.length > 0 && (
        <Field label="Save as template" hint="optional">
          <div className="flex gap-2">
            <input
              type="text"
              className="flex-1 min-w-0 rounded-lg border border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-800 px-3 py-1.5 text-xs text-stone-800 dark:text-stone-200 placeholder:text-stone-400 focus:outline-none focus:ring-1 focus:ring-teal-500"
            ...
            <button
              ...
              className="rounded-lg border border-teal-300 dark:border-teal-700 bg-teal-50 dark:bg-teal-950/20 px-3 py-1.5 text-xs font-medium text-teal-700 dark:text-teal-400 hover:bg-teal-100 dark:hover:bg-teal-950/40 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex-shrink-0"
            >
              {saving ? "Saving…" : "Save"}
            </button>
          </div>
          {saveMsg && (
            <p className="text-[10px] text-teal-600 dark:text-teal-400 mt-1">{saveMsg}</p>
          )}
        </Field>
      )}
      ...
      <div className="pt-1">
        {isApproved ? (
          <div className="rounded-lg border border-teal-200 dark:border-teal-800 bg-teal-50 dark:bg-teal-950/20 px-3 py-2.5 flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-teal-700 dark:text-teal-400">
                Schema approved
              </p>
              <p className="text-[10px] text-teal-600/70 dark:text-teal-500/70 mt-0.5">
            ...
            <button
              ...
              className="text-[10px] text-teal-500 hover:text-teal-700 dark:hover:text-teal-300 font-medium transition-colors flex-shrink-0 ml-3"
            >
              Edit
            </button>
          </div>
