...
def _get_or_404(db: Session, template_id: uuid.UUID, org_id: uuid.UUID):
    t = template_repo.get_template(db, template_id, org_id=org_id)
    if t is None:
...
def list_templates(
    ...
    templates = template_repo.list_templates(db, org_id=org_id)
...
def create_template(
    ...
    t = template_repo.create_template(db, body, org_id=org_id)
...
def get_template(
    ...
    t = _get_or_404(db, template_id, org_id)
    return SchemaTemplateRead.model_validate(t)
